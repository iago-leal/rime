#!/usr/bin/env bash
# test-traceability-check.sh — regressão de .ci/traceability-check.sh
#
# Cobre:
...
ROOT="$SCRIPT_DIR/../.."
SCRIPT="$ROOT/.ci/traceability-check.sh"
...
setup_fixture() {
  ...
  mkdir -p "$fixture/traceability" "$fixture/docs/adr" "$fixture/.ci"
  cp "$SCRIPT" "$fixture/.ci/traceability-check.sh"
  chmod +x "$fixture/.ci/traceability-check.sh"
  for f in code-spec spec-impact spec-test changelog; do
    cat > "$fixture/traceability/$f.md" <<EOF
# $f
## Convenção
(meta-doc)
EOF
...
- impacto em x: 🟩
EOF
cat >> "$F/traceability/code-spec.md" <<'EOF'

### consistente
- arquivo: src/x.ts
EOF
output=$( cd "$F" && ./.ci/traceability-check.sh 2>&1 || true )
exit_code=$( cd "$F" && ./.ci/traceability-check.sh >/dev/null 2>&1; echo $? )
