<!-- Gerado de .skills/bootstrap.md por bin/sync-skills.sh — não editar diretamente -->
...
Carregue: `.skills/_principles.md`, `.skills/_glossary.md`, `.skills/_self_check.md`.
...
4. **Delegação estrita de setup**: produz `ARCHITECTURE.md` em prosa; setup técnico (npm/poetry init, git init, lock) é delegado a `bin/setup` ou comando explícito do harness.
...
7. **ADR-0001 caso-base**: gera `docs/adr/0001-adoption-rime-v6.md` **diretamente** (sem invocar `decisao`). Greenfield exception: matriz nasce vazia; `decisao` opera sobre matriz existente. ADRs subsequentes vão por `decisao`.
...
9. **Hook pre-commit em modo B**: quando D0=B, instala `.git/hooks/pre-commit` chamando `bin/sync-skills.sh --check`. Flag `--no-hooks` opta-out. Em A/C, CI cobre.
...
| 6 | Definir estrutura inicial: `src/`, `tests/`, `registro/`, `traceability/`, `docs/`, `.skills/`, `.ci/` | layout |
...
| 12 | Inicializar `traceability/{code-spec,spec-impact,spec-test,changelog}.md` (vazios) | commits |
| 13 | Gerar `docs/adr/0001-adoption-rime-v6.md` direto (caso-base; ver invariante 7) | commit |
| 14 | Se D0=A ou D0=C: gerar `addon-pipeline/` (prompts/workflows/evals dos agentes runtime) | commits |
| 15 | Se D0=B: instalar hook pre-commit (`bin/sync-skills.sh --check`) — salvo `--no-hooks` | hook |
...
- `docs/adr/0001-adoption-rime-v6.md`.
- `traceability/*.md` inicializados.
