...
- `/decisao adr <título>` — gerar scaffold de ADR numerado em `docs/adr/`.
...
- `/execucao gate` — rodar smoke test da matriz (`.ci/traceability-check.sh`).
...
- `/operacao lista` — listar problemas ativos em `registro/lista-problemas.md`.
- `/operacao revisar` — revisar problemas, mover ativos↔passivos, classificar severidade.
- `/operacao fechar` — fechar sessão; gera selo longitudinal e atualiza `registro/sessoes/`.
...
- `bin/traceability-check` — smoke test em CI; valida invariantes da matriz tripartite (≤10s).
- `bin/sync-skills` — gerar adaptadores por harness a partir de `.skills/`.
