...
# Pré-condições
[[ -d "$TRACE_DIR" ]] || { fail "traceability/ não existe"; exit 1; }
for f in code-spec.md spec-impact.md spec-test.md changelog.md; do
  [[ -f "$TRACE_DIR/$f" ]] || fail "traceability/$f ausente"
done
(( EXIT == 0 )) || exit $EXIT
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  if [[ -n "$matrix_changes" && -z "$changelog_changes" ]]; then
    fail "mudança em matriz sem entrada em traceability/changelog.md"
  fi
fi
