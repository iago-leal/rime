...
│   ├── passivos.md
│   └── sessoes/
├── .rime/tests/regressao/             # MVES quando aplicável
├── .ci/
...
- **Sincronizar adaptadores**: `.rime/bin/sync-skills.sh` (regenera `.claude/skills/`, `.agents/skills/`, `.harness-generic/`).
- **Verificar sincronia**: `.rime/bin/sync-skills.sh --check` (CI gate).
- **Smoke test da matriz**: `.rime/ci/traceability-check.sh` (≤10s).
- **Skills**: ver `.rime/skills/_commands.md`.
