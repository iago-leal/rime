#!/usr/bin/env bash
# sync-skills.sh — gera adaptadores de .rime/skills/*.md para cada harness alvo.
#
# Uso:
#   .rime/bin/sync-skills.sh             # gera/atualiza adaptadores
#   .rime/bin/sync-skills.sh --check     # apenas verifica sincronia (CI gate)
#   .rime/bin/sync-skills.sh --inline    # inlina canônicos no início de cada adaptador
#
# Compatível com Bash 3.2+ (default macOS) — sem associative arrays.

set -euo pipefail

SOURCE_DIR=".rime/skills"
HEADER_TPL="<!-- Gerado de .rime/skills/<NOME>.md por .rime/bin/sync-skills.sh — não editar diretamente -->"
