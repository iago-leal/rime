#!/usr/bin/env bash
# test-sync-skills.sh — regressão de .rime/bin/sync-skills.sh
#
# Cobre:
...
ROOT="$SCRIPT_DIR/../../.."
SCRIPT="$ROOT/.rime/bin/sync-skills.sh"
...
setup_fixture() {
  ...
  mkdir -p "$fixture/.rime/skills"
  cp "$ROOT/.rime/skills/_principles.md" "$fixture/.rime/skills/_principles.md"
  cp "$ROOT/.rime/skills/_glossary.md" "$fixture/.rime/skills/_glossary.md"
  cp "$ROOT/.rime/skills/_self_check.md" "$fixture/.rime/skills/_self_check.md"
  cp "$ROOT/.rime/skills/captura.md" "$fixture/.rime/skills/captura.md"
...
F="$(setup_fixture)"
( cd "$F" && SOURCE_DIR=.rime/skills ./sync-skills.sh ) >/dev/null 2>&1 \
  && pass "geração ok" \
  || fail "geração falhou (regressão de Bash 3.2 compat?)"

# Caso 2: --check passa logo após geração
echo "2. --check após geração"
( cd "$F" && SOURCE_DIR=.rime/skills ./sync-skills.sh --check ) >/dev/null 2>&1 \
  && pass "--check verde" \
...
F2="$(setup_fixture)"
( cd "$F2" && SOURCE_DIR=.rime/skills ./sync-skills.sh --inline ) >/dev/null 2>&1
...
if [[ ! -f "$F/.claude/skills/_principles/SKILL.md" && ! -f "$F/.claude/skills/_glossary/SKILL.md" ]]; then
  pass "canônicos preservados em .rime/skills/"
else
